...
namespace hds
{
	...
	class AuthSocket{
    	...
    	public AuthSocket(){
			...
			Console.WriteLine("Auth server set and ready at port 11000");
		...
		private void HandleClientComm(object client){
			...
			while (working){
				...
				try{
					...
					if (auth.getStatus() ==-1){
						Console.WriteLine("World List sent, closing connection.");	
						break;
					}

				}catch(AuthException authE){
					Console.WriteLine(authE);
					break;
				}
				
  			}

  			Console.WriteLine("Auth thread closing");
  			tcpClient.Close();
			clientStream.Close();
		}	
	}	
